...
1. Define named presets in `oh-my-opencode-air.jsonc` under the `presets` field
...
| Method | How | Persists? |
|--------|-----|-----------|
| Config file | Set `"preset": "cheap"` in `oh-my-opencode-air.jsonc` | Yes, across restarts |
