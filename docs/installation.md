...
Complete installation instructions for oh-my-opencode-air.
...
```bash
bunx oh-my-opencode-air@latest install --no-tui --skills=yes
```
...
| `--preset=<name>` | Active generated config preset: `onprem` (default: `onprem`) |
...
By default, the installer is non-destructive. If an `oh-my-opencode-air.json` configuration file already exists, the installer will **not** overwrite it. Instead, it will display a message:

```
[i] Configuration already exists at ~/.config/opencode/oh-my-opencode-air.json. Use --reset to overwrite.
```

To force overwrite of your existing configuration, use the `--reset` flag:

```bash
bunx oh-my-opencode-air@latest install --reset
```
...
The installer generates an `onprem` preset active by default. To switch providers later or build a mixed setup, use **[Configuration Reference](configuration.md)** for the full option reference and the preset docs for copyable examples.
...
Open your generated config at `~/.config/opencode/oh-my-opencode-air.json`
...
> **💡 Tip: Models are fully customizable.** The installer sets sensible defaults, but you can assign *any* model to *any* agent. Edit `~/.config/opencode/oh-my-opencode-air.json` (or `.jsonc` for comments support) to override models, adjust reasoning effort, or disable agents entirely.
...
https://raw.githubusercontent.com/alvinunreal/oh-my-opencode-air/refs/heads/master/README.md
```
...
If you're an LLM Agent helping set up oh-my-opencode-air, follow these steps.
...
The installer generates an `onprem` preset active by default:
...
# Make the generated onprem preset active
bunx oh-my-opencode-air@latest install --preset=onprem

# Non-interactive without skills
bunx oh-my-opencode-air@latest install --no-tui --skills=no
...
- Generates agent model mappings in `~/.config/opencode/oh-my-opencode-air.json` (or `.jsonc`)
...
- They can easily assign **different models to different agents** by editing `~/.config/opencode/oh-my-opencode-air.json` (or `.jsonc`).
...
- Read the generated `~/.config/opencode/oh-my-opencode-air.json` (or `.jsonc`) file to understand the current configuration.
...
```bash
bunx oh-my-opencode-air@latest install --help
```

Then manually create the config files at:
- `~/.config/opencode/oh-my-opencode-air.json` (or `.jsonc`)
...
2. From your project root, verify your config file exists and is valid:
   ```bash
   bunx oh-my-opencode-air@latest doctor
   ```
...
3. Verify your config file has the correct provider configuration:
   ```bash
   cat ~/.config/opencode/oh-my-opencode-air.json
   ```
...
```jsonc
{
  "$schema": "https://unpkg.com/oh-my-opencode-air@latest/oh-my-opencode-air.schema.json",
  // your config...
}
```
...
1. **Remove the plugin from your OpenCode config**:
   ...
   Edit `~/.config/opencode/opencode.json` and remove `"oh-my-opencode-air"` from the `plugin` array.

2. **Remove configuration files (optional)**:
   ```bash
   rm -f ~/.config/opencode/oh-my-opencode-air.json
   rm -f ~/.config/opencode/oh-my-opencode-air.json.bak
   ```
