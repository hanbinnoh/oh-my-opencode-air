...
Skills are installed via the `oh-my-opencode-air` installer or manually with `npx skills add`.
...
Control which skills each agent can use in `~/.config/opencode/oh-my-opencode-air.json` (or `.jsonc`):
