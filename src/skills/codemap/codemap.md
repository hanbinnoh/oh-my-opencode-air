...
- Generate and evolve change-aware codemap state artifacts (`.air/codemap.json`) and scaffold placeholders (`codemap.md`).
...
- Persistence model: JSON state at `.air/codemap.json` with `metadata`, `file_hashes`, and `folder_hashes`.
...
- `cmdInit()` computes include/exclude candidate sets using `selectFiles()` and writes:
  1) `.air/codemap.json` via `saveState()`
...
- `codemap` skill invocation path in SKILL workflow is explicit: Step 1 checks `.air/codemap.json` or `.air/cartography.json`, then Step 2/3 selects init or incremental path.
