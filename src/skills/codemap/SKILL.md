...
**First, check if `.air/codemap.json` exists in the repo root.**

If it does not exist, check for legacy state at `.air/cartography.json`.

If legacy state exists: move `.air/cartography.json` to `.air/codemap.json`, then continue with change detection.

If `.air/codemap.json` exists: Skip to Step 3 (Detect Changes) - no need to re-initialize.
...
This creates:
- `.air/codemap.json` - File and folder hashes for change detection
