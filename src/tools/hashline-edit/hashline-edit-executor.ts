...
export async function executeHashlineEditTool(args: HashlineEditArgs, context: ToolContext, pluginCtx?: PluginInput): Promise<string> {
  try {
    ...
    if (pluginCtx?.client) {
      ...
      if (formattedContent !== writeContent) {
        ...
        context.metadata(formattedMeta)
